...
require "./1brc/macros"
require "./1brc/stat"
require "./1brc/fx_hash_map"
require "./1brc/byte_string_ops64"
require "./1brc/byte_string_ops32"
...
module OneBRCParallel
  ...
  alias Collector = FxHashMap(Metric)
  ...
  file_parts = if file_size > part_size
                 (file_size // part_size) + (file_size % part_size).clamp(0, 1).to_i32
               else
                 1
               end
    ...
    spawn do
      ...
      p_ix.step to: file_parts, by: PARALLEL_MAX, exclusive: true do |ix|
        ...
        begin
          ...
          # Use unsafe pointer when scanning the buffer ... safely
          ptr = buf.to_unsafe
          ...
          while read_pos < size
            ...
            #   read_pos &+= 1
            # end
            read_pos &+= ByteStrOps64.find_byte_by8(';', ptr + start, 32)
            # read_pos &+=  ByteStrOps32.find_byte_by4(';', ptr + start, 32)
